...
AccountWorker::PageAccount::PageAccount(std::string_view username, std::string_view deviceName)
{
    ...
    (void)username;
    (void)deviceName;
    _panel_username    = std::make_unique<PanelInfo>(_panel->get(), 50, "Factory Reset", "Erases Wi-Fi & all settings");
    _panel_device_name = std::make_unique<PanelInfo>(_panel->get(), 174, "Confirm", "Press the button below");
...
{
    // Local-only build: no M5Stack account, no network. Local factory reset only.
    _page_account = std::make_unique<PageAccount>("", "");
}
...
void AccountWorker::update()
{
    if (_page_account) {
        if (_page_account->isUnbindClicked()) {
            mclog::tagInfo(_tag, "factory reset clicked");
            _page_account.reset();

            // Local factory reset (no network unbind in local-only build)
