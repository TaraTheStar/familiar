...
EspVideo::EspVideo(const esp_video_init_config_t& config) {
    ...
    if (false) { /* placeholder to build the else-if chain */
    }
    ...
    // Note: in the current esp_video version, YUV422P actually outputs YUYV.
    ...
    // When ISP is enabled, it needs a few frames to initialize its parameters, so after startup capture and discard frames for 5s in the background
    xTaskCreate(
        [](void* arg) {
        ...
        if (i == 2) {
            ...
            switch (sensor_format_) {
                ...
                case V4L2_PIX_FMT_RGB565X: {
                    // Big-endian RGB565 must be converted to little-endian
                    // Currently esp_video returns RGB565 for both endiannesses and never RGB565X; this case exists for forward compatibility with future versions
                    auto src16 = (uint16_t*)mmap_buffers_[buf.index].start;
                    auto dst16 = (uint16_t*)frame_.data;
 ...
 * request as multipart/form-data to the configured explain server. The server performs AI analysis of
 * the image based on the provided question and returns the result.
 *
 * Implementation details:
 * - Encodes the JPEG on a separate thread, decoupled from the main thread
 * - Uses chunked transfer encoding to optimize memory usage
 * - Synchronizes data between the encoder thread and the sender thread via a queue
 * - Supports HTTP header configuration for device ID, client ID, and auth token
 *
 * @param question The question about the image to ask the AI, sent as a form field
 * @return std::string The JSON-formatted response string returned by the server
 ...
 * @note The function waits for the previous encoder thread to finish before starting new processing
...
std::string EspVideo::Explain(const std::string& question) {
    ...
    // Configure the HTTP client using chunked transfer encoding
    ...
    {
        // Part two: the file field header
    ...
    {
        // Part four: the multipart footer
