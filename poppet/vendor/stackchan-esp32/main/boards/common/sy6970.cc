...
int Sy6970::GetBatteryLevel() {
    int level = 0;
    // Minimum voltage the battery can discharge to
    ...
    // When no battery is connected, the charge status read is unstable and battery_voltage sometimes exceeds charge_voltage_limit
    if (level > 100) {
        level = 100;
    }
    return level;
