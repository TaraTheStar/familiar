...
#if CONFIG_STACKCHAN_ENABLE_HARDWARE_JPEG_ENCODER
...
static uint8_t* convert_input_to_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    // V4L2 UYVY (Cb Y Cr Y) -> reordered to YUYV, then fed as YCbYCr input
    // The UYVY format does not currently occur in this version
    ...
    // V4L2 YUV422P (YUV422 Planar) -> reordered to YUYV (YCbYCr)
    // The YUV422P format does not currently occur in this version
    ...
    // See https://github.com/78/xiaozhi-esp32/issues/1380#issuecomment-3497156378
        ...
        switch (format) {
            ...
            case V4L2_PIX_FMT_RGB565:
                ...
                break;
            [[unlikely]] case V4L2_PIX_FMT_RGB565X: // RGB565X does not currently occur in this version
...
static uint8_t* convert_input_to_hw_encoder_buf(const uint8_t* src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    if (format == V4L2_PIX_FMT_YUYV) {
        // The hardware requires the "big-endian" | Y1 V Y0 U | format, so bswap16 is needed
        int sz = (int)width * (int)height * 2;
