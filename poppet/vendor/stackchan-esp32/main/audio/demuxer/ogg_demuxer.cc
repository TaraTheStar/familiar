    ...
    while (processed < size) {
        switch (state_) {
          case ParseState::FIND_PAGE: {
            // Look for the page header "OggS"
            if (ctx_.bytes_needed < 4) {
                // Handle an incomplete "OggS" match (spanning data chunks)
                ...
                if (ctx_.bytes_needed == 0) {
                    ...
                    if (memcmp(ctx_.header, "OggS", 4) == 0) {
                        ...
                        // Match failed; slide forward 1 byte and keep matching
                ...
                // Search for a complete "OggS" within the data chunk
                bool found = false;
                size_t i = 0;
                ...
                if (found) {
                    ...
                    // No complete "OggS" found; save the possible partial match
          ...
          case ParseState::PARSE_HEADER: {
            ...
            if (available < ctx_.bytes_needed) {
                ...
                if (ctx_.seg_count > 0 && ctx_.seg_count <= 255) {
                    ...
                    // No segments; jump straight to the next page
                    state_ = ParseState::FIND_PAGE;
                    ctx_.bytes_needed = 4;
                    ctx_.data_offset = 0;
                } else {
          ...
          case ParseState::PARSE_DATA: {
            while (ctx_.seg_index < ctx_.seg_count && processed < size) {
                ...
                // Check whether the segment data has already been partially read
                ...
                // Check whether the buffer has enough room
                ...
                if (!seg_continued) {
                    ...
                    if (ctx_.packet_len) {
                        ...
                        if (opus_info_.head_seen && opus_info_.tags_seen) {
                            ...
                            ESP_LOGW(TAG, "OpusHead/OpusTags not parsed from the current Ogg container; discarding");
                        }
                    }
                    ctx_.packet_len = 0;
                    ctx_.packet_continued = false;
